...
// at) + a push notification. Wrapped so a failure here can never fail the
// candidate creation/approval that triggered it. Shared between
// adminMatrimony.ts (direct create) and adminMatrimonyApplications.ts
// (approve-from-application), the two paths a new candidate can appear from.
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO portal_notifications (recipient_id, actor_id, type, message)
       SELECT membership_no, membership_no, 'new_candidate', $1
       FROM members
